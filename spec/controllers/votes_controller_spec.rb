...
RSpec.describe VotesController, type: :controller do
  ...
  context 'signed in user' do
    ...
    describe 'POST up_vote' do
      it 'increases the number of post votes by one for the users first vote' do
        votes = user_post.votes.count
        post :up_vote, post_id: user_post.id
        expect(user_post.votes.count).to eq(votes + 1)
      end

      it 'should not increase the number of post votes for the users second vote' do
        post :up_vote, post_id: user_post.id
        votes = user_post.votes.count
        post :up_vote, post_id: user_post.id
    ...
    describe 'POST down_vote' do
      it 'decreases the number of post votes by one for the users first vote' do
        votes = user_post.votes.count
        post :down_vote, post_id: user_post.id
        expect(user_post.votes.count).to eq(votes + 1)
      end

      it 'should not decrease the number of post votes for the users second vote' do
        post :down_vote, post_id: user_post.id
        votes = user_post.votes.count
        post :down_vote, post_id: user_post.id
        expect(user_post.votes.count).to eq(votes)
      ...
      it 'decrease the sum of post votes by one' do
        points = user_post.points
        post :down_vote, post_id: user_post.id
        expect(user_post.points).to eq(points - 1)
      end
